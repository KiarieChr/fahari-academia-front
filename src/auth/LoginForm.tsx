    ...
    try {
      if (activeTab === 'login') {
        ...
        if (authToken) {
          ...
          // Small delay and dispatch for other components
        ...
        if (isFirstTimeUser) {
          ...
          navigate(`/first-time-setup?userId=${userId}`);
        } else {
    ...
    <div className="form-wrapper">
      ...
      {!REGISTRATION_ENABLED ? (
        <div style={{ marginBottom: '1.5rem', textAlign: 'center' }}>
          <h2 style={{ fontSize: '1.5rem', fontWeight: 700, color: '#1a237e', margin: 0 }}>Sign In</h2>
          <p style={{ color: '#9fa8da', fontSize: '0.85rem', marginTop: '0.4rem' }}>Welcome back to the portal</p>
        </div>
      ) : (
      ...
      >
        ...
        <motion.div
          ...
          style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', width: '100%', marginBottom: '0.75rem' }}
        >
          {activeTab === 'login' ? (

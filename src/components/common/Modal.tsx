...
const Modal = ({
    ...
    return (
        <div className="fixed inset-0 z-[8000] flex items-center justify-center p-4 sm:p-6 animate-in fade-in duration-200">
            ...
            <div
                ref={panelRef}
                className={`relative bg-white rounded-2xl shadow-[0_25px_60px_-12px_rgba(0,0,0,0.25)] w-full ${SIZE_MAP[size] || SIZE_MAP.md} flex flex-col overflow-hidden max-h-[90vh] animate-in zoom-in-95 slide-in-from-bottom-2 duration-200 p-2`}
                role="dialog"
                aria-modal="true"
            >
                {/* Accent top bar */}
...
Modal.CancelButton = ({ onClick, children = 'Cancel' }) => (
    <button
        ...
        className=" text-[13px] font-semibold text-gray-600 hover:text-gray-800 bg-white border border-gray-200 rounded-xl hover:bg-gray-50 hover:border-gray-300 transition-all active:scale-[0.98]"
    >
        {children}
    </button>
);
...
Modal.SubmitButton = ({ onClick, form, disabled, loading, children, label, className = '' }) => (
    <button
        ...
        className={`inline-flex items-center justify-center gap-25 text-[13px] font-semibold text-white bg-indigo-600 rounded-xl hover:bg-indigo-700 shadow-sm shadow-indigo-200/60 transition-all disabled:opacity-50 active:scale-[0.98] ${className}`}
    >
        {loading && (

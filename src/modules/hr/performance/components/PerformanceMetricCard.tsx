...
const PerformanceMetricCard = ({ title, value, trend, trendUp, icon, color, description }) => {
    const colorMap = {
    ...
    // If color is not in map (e.g. from backend passing 'bg-blue-500'), fallback to generic or parse it
    let baseColor = color;
    if (color && color.startsWith('bg-')) {
        baseColor = color.split('-')[1]; // e.g. 'bg-blue-500' -> 'blue'
    }

...
const AttendancePolicySettings = () => {
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            setCampuses(campusesRes.results || campusesRes || []);
    ...
    const handleSaveProfile = async () => {
        try {
            ...
            if (editingProfile) {
                await api.put(`/workforce/api/employee-attendance-access-profiles/${editingProfile.id}/`, profileForm);
                toast.success('Employee attendance profile updated successfully');
            } else {
                await api.post('/workforce/api/employee-attendance-access-profiles/', profileForm);
    ...
    const handleUpdateCampus = async (campusId) => {
        try {
            if (!campusForm.latitude || !campusForm.longitude) {
            ...
            await api.patch(`/workforce/api/campuses/${campusId}/`, {
                ...
                geofence_radius_meters: parseInt(campusForm.geofence_radius_meters),
            });
            toast.success('Campus geofence updated successfully');
            setCampusForm({ name: '', latitude: '', longitude: '', geofence_radius_meters: 200 });
            fetchData();
        } catch (error) {
            console.error(error);
                            ...
                            <div key={campus.id} className="bg-white border border-slate-200 rounded-lg p-4 space-y-4">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                    <input
                                        ...
                                        onChange={(e) => setCampusForm({ ...campusForm, name: campus.name, latitude: e.target.value })}
                                        className="px-3 py-2 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
                                    />
                                    <input
                                        type="number"
                                        ...
                                        onChange={(e) => setCampusForm({ ...campusForm, name: campus.name, longitude: e.target.value })}
                                        className="px-3 py-2 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
                                    />
                                    <input
                                        type="number"
                                        placeholder="Radius (meters)"
                                        defaultValue={campus.geofence_radius_meters || 200}
                                        onChange={(e) => setCampusForm({ ...campusForm, name: campus.name, geofence_radius_meters: e.target.value })}
                                        className="px-3 py-2 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
                                    />
                                </div>

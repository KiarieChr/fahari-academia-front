...
const LeaveCalendar = ({ requests }) => {
    ...
    const getLeaveForDate = (day) => {
        ...
        return requests.filter(req => {
            ...
            const statusStr = (req.status_display || req.status || '').toLowerCase();
            return check >= start && check <= end && statusStr.includes('approved');
        });
    };
                                    ...
                                    <div className="space-y-1">
                                        {leaves.map((leave, idx) => (
                                            <div
                                                ...
                                                title={`${leave.employee_name || leave.employee} - ${leave.leave_type_name || leave.type}`}
                                            >
                                                {(leave.employee_name || leave.employee || '').split(' ')[0]}
                                            </div>
                                        ))}
                                        {leaves.length > 2 && (

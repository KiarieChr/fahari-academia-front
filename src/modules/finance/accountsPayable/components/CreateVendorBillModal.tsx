...
const CreateVendorBillModal = ({ show, onClose, onSave }) => {
    ...
    const loadDependencies = async () => {
        try {
            ...
            // Filter for EXPENSE or LIABILITY accounts (expense for line items, liability for trade payables)
            const allAccounts = accRes?.results || accRes || [];
            const expAccs = allAccounts.filter(acc => ['EXPENSE', 'LIABILITY'].includes(acc.type));

            ...
            <div className="fr-dashboard">
                ...
                <div className="row g-4">
                    ...
                    <div className="col-12 col-md-4 col-lg-3">
                        <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 p-3 shadow-sm d-flex flex-column gap-1 sticky-top overflow-y-auto custom-scrollbar" style={{ top: '6rem', maxHeight: 'calc(100vh - 8rem)' }}>
                            {TABS.map((tab) => {
                                const Icon = tab.icon;
                                ...
                                return (
                                    <button
                                        ...
                                        className={`d-flex align-items-center gap-3 px-3 py-2 text-sm fw-medium rounded-3 transition-all w-100 text-start border-0 ${isActive
                                            ? 'bg-primary-subtle text-primary dark:bg-blue-900/30'
                                            : 'text-secondary bg-transparent hover-bg-light dark:text-slate-400 dark:hover:bg-slate-700/50'
                                        }`}
                                    >
                                        <Icon size={18} />
                                        {tab.label}
                    ...
                    <div className="col-12 col-md-8 col-lg-9">
                        <div className="report-content-area bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 p-4 shadow-sm" style={{ minHeight: '500px' }}>
                            {activeTab === 'overview' && (
                                <>
                                    {/* Executive Summary Snapshot */}
                                    <div className="fr-kpi-grid mb-4">
                                        {kpiCards.map((card, idx) => (
                                            <div key={idx} className="fr-kpi-card border border-slate-100 shadow-sm rounded-xl p-4">
                                                <div className="d-flex justify-content-between align-items-start mb-2">
                                                    <span className="text-muted text-xs font-semibold uppercase tracking-wider">{card.label}</span>
                                                    {card.icon}
                                                </div>
                                                <div className="text-2xl font-bold mb-2 text-slate-800 dark:text-white">{card.value}</div>
                                    ...
                                    <h3 className="text-lg font-bold text-slate-800 dark:text-white mb-4 mt-6">All Reports</h3>

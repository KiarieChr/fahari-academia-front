                ...
                <div className="d-flex gap-2">
                    ...
                    {data && (
                        <ExportButton 
                            ...
                            getRows={() => {
                                const rows = [];
                                if (data.revenue?.lines) {
                                    rows.push(['REVENUE', '', '']);
                                    data.revenue.lines.forEach(l => rows.push(['', l.name, l.balance]));
                                    rows.push(['Total Revenue', '', data.revenue.total]);
                                }
                                if (data.expenses?.lines) {
                                    rows.push(['EXPENSES', '', '']);
                                    data.expenses.lines.forEach(l => rows.push(['', l.name, l.balance]));
                                    rows.push(['Total Expenses', '', data.expenses.total]);
                                }
                                rows.push(['NET INCOME', '', data.summary?.net_income || 0]);
                                return rows;
                            }}

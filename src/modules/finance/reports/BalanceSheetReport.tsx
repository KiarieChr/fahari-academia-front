                ...
                <div className="d-flex gap-2">
                    ...
                    {data && (
                        <ExportButton 
                            ...
                            getRows={() => {
                                const rows = [];
                                if (data.assets?.lines) {
                                    rows.push(['ASSETS', '', '']);
                                    data.assets.lines.forEach(l => rows.push(['', l.name, l.balance]));
                                    rows.push(['Total Assets', '', data.assets.total]);
                                }
                                if (data.liabilities?.lines) {
                                    rows.push(['LIABILITIES', '', '']);
                                    data.liabilities.lines.forEach(l => rows.push(['', l.name, l.balance]));
                                    rows.push(['Total Liabilities', '', data.liabilities.total]);
                                }
                                if (data.equity?.lines) {
                                    rows.push(['EQUITY', '', '']);
                                    data.equity.lines.forEach(l => rows.push(['', l.name, l.balance]));

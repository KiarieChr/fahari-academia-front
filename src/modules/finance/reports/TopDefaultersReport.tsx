...
const TopDefaultersReport = () => {
    ...
    const [filters, setFilters] = useState({ academic_year: '', term: '' });
    const [terms, setTerms] = useState([]);
    const [academicYears, setAcademicYears] = useState([]);

    const fetchConfig = async () => {
        try {
            const [tRes, ayRes] = await Promise.all([
                studentSettingsService.getTerms(),
                studentSettingsService.getAcademicYears()
            ]);
            setTerms(tRes?.results || tRes || []);
    ...
    const fetchData = async () => {
        setLoading(true);
        try {
            const res = await financeService.getTopDefaulters({ limit, ...filters });
            setData(res);
        } catch { }
        setLoading(false);
    };
    ...
    useEffect(() => { fetchData(); }, [limit, filters.academic_year, filters.term]);
        ...
        <div className="space-y-5 animate-in fade-in duration-500">
            ...
            <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm p-5">
                <div className="flex flex-wrap gap-3 items-end">
                    <div>
                        <label className="text-xs text-slate-500 font-medium block mb-1">Academic Year</label>
